<nav class="border-b border-gray-200 bg-white"
    x-data="{ 
        showMobile: false 
    }"
>
    <div class="mx-auto max-w-7xl px-4 sm:px-6 lg:px-8">
        <div class="flex h-16 justify-between items-center">
            <div class="flex">
                <div class="flex flex-shrink-0 items-center">
                    <a href="{{ route('home') }}">
                        <x-logo class="h-8 w-8 text-indigo-600" />
                    </a>
                </div>
                <div class="hidden sm:-my-px sm:ml-6 sm:flex sm:space-x-8">
                    @foreach ($nav as $item)
                        <a href="{{ route(Arr::get($item, 'route')) }}" @class([
                            'border-indigo-500 text-gray-900' => Route::is(Arr::get($item, 'route')),
                            'border-transparent text-gray-500 hover:border-gray-300 hover:text-gray-700' => ! Route::is(Arr::get($item, 'route')),
                            'inline-flex items-center border-b-2 px-1 pt-1 text-sm font-medium' => true,
                        ])>
                            {{ Arr::get($item, 'label') }}
                        </a>
                    @endforeach
                </div>
            </div>
            @auth
                <div class="hidden sm:ml-6 sm:flex sm:items-center">
                    <div class="relative ml-3" 
                        x-data="{ 
                            show: false 
                        }"
                        x-on:click.away="show = false"
                    >
                        <div>
                            <button type="button" class="relative flex max-w-xs items-center rounded-full bg-white text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2" id="user-menu-button" aria-expanded="false" aria-haspopup="true"
                                x-on:click="show = !show"
                            >
                                <span class="absolute -inset-1.5"></span>
                                <span class="sr-only">{{ __('Open user menu') }}</span>
                                <img class="h-8 w-8 rounded-full" src="https://gravatar.com/avatar/{{ hash('sha256', auth()->user()->email) }}" alt="">
                            </button>
                        </div>
                        <div class="absolute right-0 z-10 mt-2 w-48 origin-top-right rounded-md bg-white py-1 shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none" role="menu" aria-orientation="vertical" aria-labelledby="user-menu-button" tabindex="-1"
                            x-cloak
                            x-show="show" 
                            x-transition:enter="transition ease-out duration-200"
                            x-transition:enter-start="transform opacity-0 scale-95"
                            x-transition:enter-end="transform opacity-100 scale-100"
                            x-transition:leave="transition ease-in duration-75"
                            x-transition:leave-start="transform opacity-100 scale-100"
                            x-transition:leave-end="transform opacity-0 scale-95"
                        >
                            <a href="{{ route('filament.admin.resources.recipes.index') }}" class="block px-4 py-2 text-sm text-gray-700">
                                {{ __('Añadir receta') }}
                            </a>
                            <a href="{{ route('logout') }}" @class([
                                'bg-gray-100' => Route::is('logout'),
                                'block px-4 py-2 text-sm text-gray-700' => true,
                            ])>
                                {{ __('Log out') }}
                            </a>
                        </div>
                    </div>
                </div>
            @else
                <div class="hidden sm:-my-px sm:ml-6 sm:flex sm:space-x-8">
                    @if (Route::has('login'))
                        <a href="{{ route('login') }}" @class([
                            'bg-gray-100' => Route::is('login'),
                            'block px-4 py-2 text-sm text-gray-700' => true,
                        ])>
                            {{ __('Log in') }}
                        </a>
                    @endif
                    @if (Route::has('register'))
                        <a href="{{ route('register') }}" @class([
                            'bg-gray-100' => Route::is('register'),
                            'block px-4 py-2 text-sm text-gray-700' => true,
                        ])>
                            {{ __('Register') }}
                        </a>
                    @endif
                </div>
            @endif
            <div class="-mr-2 flex items-center sm:hidden">
                <!-- Mobile menu button -->
                <button type="button" class="relative inline-flex items-center justify-center rounded-md bg-white p-2 text-gray-400 hover:bg-gray-100 hover:text-gray-500 focus:outline-none focus:ring-2 focus:ring-indigo-500 focus:ring-offset-2" aria-controls="mobile-menu" aria-expanded="false"
                        x-on:click="showMobile = !showMobile"
                >
                    <span class="absolute -inset-0.5"></span>
                    <span class="sr-only">{{ __('Open main menu') }}</span>
                    <!-- Menu open: "hidden", Menu closed: "block" -->
                    <svg x-show="!showMobile" class="block h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M3.75 6.75h16.5M3.75 12h16.5m-16.5 5.25h16.5" />
                    </svg>
                    <!-- Menu open: "block", Menu closed: "hidden" -->
                    <svg x-show="showMobile" class="block h-6 w-6" fill="none" viewBox="0 0 24 24" stroke-width="1.5" stroke="currentColor" aria-hidden="true">
                        <path stroke-linecap="round" stroke-linejoin="round" d="M6 18L18 6M6 6l12 12" />
                    </svg>
                </button>
            </div>
        </div>
    </div>

    <!-- Mobile menu, show/hide based on menu state. -->
    <div class="sm:hidden" id="mobile-menu"
        x-cloak
        x-show="showMobile" 
        x-on:click.away="showMobile = false" 
    >
        <div class="space-y-1 pb-3 pt-2">
            @foreach ($nav as $item)
                <a href="{{ route(Arr::get($item, 'route')) }}" @class([
                    'border-indigo-500 bg-indigo-50 text-indigo-700' => Route::is(Arr::get($item, 'route')),
                    'border-transparent text-gray-600 hover:border-gray-300 hover:bg-gray-50 hover:text-gray-800' => ! Route::is(Arr::get($item, 'route')),
                    'block border-l-4 py-2 pl-3 pr-4 text-base font-medium' => true,
                ])>
                    {{ Arr::get($item, 'label') }}
                </a>
            @endforeach
            @auth
                <a href="{{ route('filament.admin.resources.recipes.index') }}" @class([
                    'border-indigo-500 bg-indigo-50 text-indigo-700' => Route::is('filament.admin.resources.recipes.index'),
                    'border-transparent text-gray-600 hover:border-gray-300 hover:bg-gray-50 hover:text-gray-800' => ! Route::is('filament.admin.resources.recipes.index'),
                    'block border-l-4 py-2 pl-3 pr-4 text-base font-medium' => true,
                ])>
                    {{ __('Añadir receta') }}
                </a>
                <a href="{{ route('logout') }}" @class([
                    'border-indigo-500 bg-indigo-50 text-indigo-700' => Route::is('logout'),
                    'border-transparent text-gray-600 hover:border-gray-300 hover:bg-gray-50 hover:text-gray-800' => ! Route::is('logout'),
                    'block border-l-4 py-2 pl-3 pr-4 text-base font-medium' => true,
                ])>
                    {{ __('Sign out') }}
                </a>
            @else
                @if (Route::has('login'))
                    <a href="{{ route('login') }}" @class([
                        'border-indigo-500 bg-indigo-50 text-indigo-700' => Route::is('login'),
                        'border-transparent text-gray-600 hover:border-gray-300 hover:bg-gray-50 hover:text-gray-800' => ! Route::is('login'),
                        'block border-l-4 py-2 pl-3 pr-4 text-base font-medium' => true,
                    ])>
                        {{ __('Log in') }}
                    </a>
                @endif
                @if (Route::has('register'))
                    <a href="{{ route('register') }}" @class([
                        'border-indigo-500 bg-indigo-50 text-indigo-700' => Route::is('register'),
                        'border-transparent text-gray-600 hover:border-gray-300 hover:bg-gray-50 hover:text-gray-800' => ! Route::is('register'),
                        'block border-l-4 py-2 pl-3 pr-4 text-base font-medium' => true,
                    ])>
                        {{ __('Register') }}
                    </a>
                @endif
            @endif
        </div>
    </div>
</nav>